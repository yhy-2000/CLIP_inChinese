...
class SentenceModel(tf.keras.Model):
    ...
    def generateSingleEmbedding(self, input, training=False):
        ...
        outAtt = tf.cast(att, tf.float32) # 强制类型转换,转换成float32的格式
        sampleLength = tf.reduce_sum(outAtt, axis=-1, keepdims=True) # 求整体的和
        maskedEmbs = embs * tf.expand_dims(outAtt, axis=-1) # 在最后一维之后再加一维
